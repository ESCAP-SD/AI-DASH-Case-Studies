...
if (nrow(cs) == 0) {
  ...
  details <- pmap_chr(
    ...
    function(business_need, description, benefits, challenges,
      ...
      # Remove common trailing punctuation that people add after URLs
      urls <- gsub("[\\.,;:!\\?]+$", "", urls)

      # Also handle closing brackets/quotes that often appear after pasted URLs
      urls <- gsub("[\\)\\]\\}]+$", "", urls)
      urls <- gsub("[\"']+$", "", urls)

      # Re-unique after cleaning
      urls <- unique(urls)
